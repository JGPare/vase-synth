...
export default function ControlPanel({ meshRef, spinSpeed, setSpinSpeed }) {
  ...
  const toggleModifierEnabled = (i) => {
    if (!vaseData) return
    const newData = JSON.parse(JSON.stringify(vaseData))
    const isEnabled = newData.modifiers[i].enabled !== false
    newData.modifiers[i].enabled = !isEnabled
    setVaseData(newData)
  }
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <button
                  onClick={() => toggleModifierEnabled(i)}
                  className={`px-2 py-0.5 rounded text-xs w-9 text-center ${isEnabled ? 'bg-green-700/60 text-green-200 hover:bg-green-700' : 'bg-gray-800 text-gray-500 hover:bg-gray-700'}`}
                  title={isEnabled ? 'disable modifier' : 'enable modifier'}
                >
                  {isEnabled ? 'on' : 'off'}
                </button>
                <select
              ...
              <div className={`space-y-1 ${isEnabled ? '' : 'opacity-40'}`}>
                {mod.type === 'julia_edge_find' || mod.type === 'julia_radial' ? (
                  <button
